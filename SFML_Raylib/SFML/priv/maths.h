...
struct TVec2 {
    ...
    TVec2(const rl::Vector2 &v) : x((T)v.x), y((T)v.y) {}
    template<typename Q>
    ...
    operator rl::Vector2() const { return { (float)x, (float)y }; }
};
...
struct TVec3 {
    ...
    TVec3(const rl::Vector3 &v) : x(v.x), y(v.y), z(v.z) {}
    template<typename Q>
    ...
    operator rl::Vector3() const { return { (float)x, (float)y, (float)z }; }
};
...
struct TRect {
    ...
    TRect(const rl::Rectangle &r) : x(r.x), y(r.y), w(r.width), h(r.height) {}
    template<typename Q>
    TRect(const TRect<Q> &v) : x((T)v.x), y((T)v.y), w((T)v.w), h((T)v.h) {}

    TVec2<T> centre() const { return pos + size / (T)2; }
    bool intersects(const TRect &v) const { return rl::CheckCollisionRecs(*this, v); }
    bool contains(TVec2<T> p) const { return rl::CheckCollisionPointRec(p, *this); }
    ...
    operator rl::Rectangle() const { return { (float)x, (float)y, (float)w, (float)h }; }
};
...
struct Matrix {
    ...
    Matrix() : rl_mat(rl::MatrixIdentity()) {}
    Matrix(const rl::Matrix &m) : rl_mat(m) {}

    void translate(TVec2<float> pos) { rl_mat = rl::MatrixTranslate(pos.x, pos.y, 0.f); }
    void scale(TVec2<float> s) { rl_mat = rl::MatrixScale(s.x, s.y, 0.f); }
    void rotateX(float angle) { rl_mat = rl::MatrixRotateX(angle); }
    void rotateY(float angle) { rl_mat = rl::MatrixRotateY(angle); }
    void rotateZ(float angle) { rl_mat = rl::MatrixRotateZ(angle); }
    void rotate(float angle) { rotateZ(angle); }

    TVec2<float> transformPoint(TVec2<float> p) const { return rl::Vector2Transform(p, rl_mat); }

    const float *getMatrix() const { return data; }
    
    Matrix operator+(const Matrix &m) const { return rl::MatrixAdd(rl_mat, m.rl_mat); }
    Matrix operator-(const Matrix &m) const { return rl::MatrixSubtract(rl_mat, m.rl_mat); }
    Matrix operator*(const Matrix &m) const { return rl::MatrixMultiply(rl_mat, m.rl_mat); }

    Matrix &operator+=(const Matrix &m) { rl_mat = rl::MatrixAdd(rl_mat, m.rl_mat); return *this; }
    Matrix &operator-=(const Matrix &m) { rl_mat = rl::MatrixSubtract(rl_mat, m.rl_mat); return *this; }
    Matrix &operator*=(const Matrix &m) { rl_mat = rl::MatrixMultiply(rl_mat, m.rl_mat); return *this; }

    float &operator[](size_t index) { return data[index]; }
    const float &operator[](size_t index) const { return data[index]; }

    operator rl::Matrix() const { return rl_mat; }
};
